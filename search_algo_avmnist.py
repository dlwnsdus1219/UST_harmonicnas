...
def flatten_config(config, parent_key='', sep='.'):
    """중첩된 wandb.config를 평탄화(flatten)된 딕셔너리로 변환"""
    flat_config = {}
    for key, value in config.items():
        ...
        if isinstance(value, dict):
            flat_config.update(flatten_config(value, new_key, sep=sep))
        else:
            flat_config[new_key] = value
    return flat_config

def main_worker():
    """Worker function for running the search with WandB sweep."""
    with wandb.init(reinit=True) as run:
        ...
        # flatten_config 함수로 wandb.config 평탄화
        flat_config = flatten_config(wandb.config)

        # # wandb.config 값으로 args 업데이트
        ...
        #     elif len(keys) == 2:
        #         if hasattr(args, keys[0]):
        #             sub_attr = getattr(args, keys[0])
        #             if isinstance(sub_attr, argparse.Namespace) and hasattr(sub_attr, keys[1]):
        #                 setattr(sub_attr, keys[1], value)
        #                 print(f"Updated {keys[0]}.{keys[1]} to {value}")

        # print(f"Updated Config for Sweep: {vars(args)}")

        # wandb.config 값으로 args 업데이트
        for key, value in flat_config.items():
            keys = key.split('.')
            target = args  # args에서 시작
            for k in keys[:-1]:  # 마지막 이전까지의 키를 반복 탐색
                if hasattr(target, k):
                    target = getattr(target, k)
                else:
                    print(f"Key not found: {k} in {keys}")
                    break
        ...
        # eval_worker 호출 (args 전달)
        eval_worker(args.gpu, 1, args)

if __name__ == '__main__':
    # Sweep 설정 정의
    # Sweep 설정 정의하기
    sweep_config = {
        'method': 'bayes',  # 탐색 방식: 'bayes', 'random', 'grid' 중 하나 선택 가능
        'metric': {'name': 'test_accuracy', 'goal': 'maximize'},  # 평가 기준
        'parameters': {
            'evo_search_outer.parent_popu_size': {'values': [6, 8, 10]},
            'evo_search_outer.survival_ratio': {'values': [0.3, 0.5, 0.7]},
            'evo_search_outer.mutate_size': {'values': [32, 64, 128]},
            'evo_search_outer.crossover_size': {'values': [32, 64, 128]},
            'evo_search_outer.mutate_prob': {'min': 0.2, 'max': 0.5},  # 연속값 탐색
            'evo_search_outer.crossover_prob': {'values': [0.7, 0.8, 0.9]},
            'evo_search_outer.evo_iter': {'values': [20, 30, 40]},

            'evo_search_inner.survival_ratio': {'values': [0.2, 0.25, 0.3]},

            'fusion_epochs': {'values': [20, 25, 30]},
            'L': {'values': [4, 9, 16, 25]}  # layer 수 조정
        }
    }
    ...
    # run_sweep에서 main_worker를 실행
    run_sweep(sweep_id, function=lambda: main_worker(), count=10)
...
#     # WandB 초기화 및 설정 불러오기def eval_worker(gpu, ngpus_per_node):
#     """Worker function for evaluating evolutionary algorithm."""
#     # WandB 초기화
#     with wandb.init(reinit=True) as run:
#         args = setup(run_args.config_file)
#         args.gpu = gpu          # local rank, local machine cuda id
...
# #     run_sweep(sweep_id, function=lambda: eval_worker(0, 1), count=20)