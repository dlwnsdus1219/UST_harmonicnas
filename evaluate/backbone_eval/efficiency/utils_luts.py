...
class EfficiencyEstimator(object):
    ...
    def predict(self, ltype: str, _input, in_ch=None, out_ch=None, mid_ch=None, 
        ...
        # LUT를 사용할 수 없으므로 기본값 반환
        if ltype in ('MBConvLayer', 'ConvLayer', 'LinearLayer'):
            return 0.85, 8.5  # 기본 latency, energy 값 (임의 설정)
...
def look_up_ofa_proxy(net, lut, resolution=224, supernet='ofa', num_channels=3):
    if lut is None:
        print("⚠️ Warning: LUT is not available. Using default latency and energy values.")
        return 1.0, 10.0  # 기본값 (임의 설정)
    ...
    # 첫 번째 Conv 레이어 예측 (LUT 기반)
    lat, enrg = lut.predict(  # 🔥 여기서 lut이 None이면 오류 발생!
        'ConvLayer',
        [1, num_channels, resolution, resolution],
        in_ch=net.first_conv.config['in_channels'],
        out_ch=net.first_conv.config['out_channels'],
