...
## 개별 개체 평가 시, Search Space Encoding!!
def validate_population(train_loader, val_loader, supernet, population, args, 
                        # lut_data, 
                        modal_num=0, bn_calibration=True, in_channels=3, resolution=28):
    new_population = []
    supernet = supernet.module \
        if isinstance(supernet, torch.nn.parallel.DistributedDataParallel) else supernet
    for indiv in population:
        ## Search Space Encoding => 백본의 ks, e, d 설정하여 Subnet 구성!!
